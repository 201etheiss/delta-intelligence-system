    ...
    <div className="rounded-lg border border-[#27272A] bg-[#18181B] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#27272A]">
        <div className="flex items-center gap-2">
          <svg className="w-4 h-4 text-[#FE5000]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {(['anomalies', 'suggestions', 'patterns'] as const).map((tab) => (
          <button
            ...
            className={`flex-1 px-3 py-2 text-[10px] font-medium uppercase tracking-wide transition-colors ${
              activeTab === tab
                ? 'text-[#FE5000] border-b-2 border-[#FE5000] bg-[#FE5000]/5'
              ...
              (data.anomalies ?? []).map((a) => (
                <div key={a.id} className="flex items-start gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    {a.recommendedAction && (
                      <p className="text-[10px] text-[#FE5000] mt-0.5">{a.recommendedAction}</p>
                    )}
                  </div>
                </div>
              ))
            ...
            {data.suggestions.length === 0 ? (
              ...
              (data.suggestions ?? []).map((s, i) => (
                <Link
                  key={`suggestion-${i}`}
                  href="/chat"
                  className="block rounded-lg border border-[#27272A] px-3 py-2 hover:border-[#FE5000]/30 hover:bg-[#27272A]/50 transition-colors"
                >
            ...
            {data.patterns.length === 0 ? (
              ...
              (data.patterns ?? []).map((p) => (
                <div key={p.id} className="border-b border-[#27272A] pb-2 last:border-0">
                  ...
                  {p.recommendedAction && (
                    <p className="text-[10px] text-[#FE5000] mt-0.5">{p.recommendedAction}</p>
                  )}
                </div>
              ))
            )}
      ...
      <Link
        href="/analytics"
        className="flex items-center justify-center gap-1.5 border-t border-[#27272A] px-4 py-2.5 text-[10px] font-medium text-[#71717A] hover:text-[#FE5000] transition-colors"
      >
